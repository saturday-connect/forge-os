...
| `POST` | `/api/projects/join` | Clone a shared docs repo into a new managed project (resolves repo by slug from the registry). A github.com clone denial returns `reason: "access_denied"` + the repo URL so the UI shows the request-access flow (GitHub reports invisible private repos as not-found — that IS the denial signal; file://, ssh, and network errors stay plain errors) |
...
`GET /api/projects` attaches a per-project `collaboration` summary (`shared`/`visibility`, `joined`, `docs_repo_url`, `role`, `last_synced_at`, `error`) read from each project's own state file; `GET /api/state` exposes the active project's full `collaboration` block.
...
```text
.forge/project-state.json       project settings, git, environments, builds, issues, collaboration (docs_repo_url, visibility, shared_at/joined_at, last_docs_repo_url + unshared_at after unshare, last_error, visibility_note when org-internal fell back to private, role + role_checked_at cached from GitHub, last_synced_at)
...
Registry hygiene (beta.123), access management (beta.124), and the sync round-trip (beta.125 — pull/push with version-snapshot overwrite protection, GitHub-permission→role gating, Sync dialog) are all shipped. Remaining nice-to-haves:
- **Sync is manual** (a Sync button); auto-sync (pull-before-open / push-after-review, fire-and-forget so it never blocks) is the natural next step — keep it advisory, like the build-cache remote sync.
