...
- **Auto-sync is half-shipped**: pull-on-open landed in beta.127 (`auto_pull_on_open`, fire-and-forget background pull on select). The remaining half is **push-after-review** — auto-push when the review phase reaches a checkpoint. Open decision before building it: the push trigger must be debounced (pushing on every single `/api/review` mark would spam N pushes); the natural checkpoint is "all docs reviewed" / a gate passing, not per-file. Keep it advisory + opt-in like pull-on-open.
