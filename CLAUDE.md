...
```text
.forge/project-state.json       project settings, git, environments, builds, issues, collaboration (docs_repo_url, visibility, shared_at/joined_at, last_docs_repo_url + unshared_at after unshare, last_error, visibility_note when org-internal fell back to private)
...
Registry hygiene shipped in beta.123 (private unlisting, write retry-on-race, mirror re-share, unshare, card pills). Access management shipped in beta.124 (invite/list collaborators from the Share dialog, org-internal visibility with private fallback, request-access dialog on denied joins). Still deferred:
