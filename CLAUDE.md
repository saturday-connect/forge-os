...
- **Auto-sync is fully shipped**: pull-on-open (beta.127, `auto_pull_on_open`) + push-after-review (beta.128, `auto_push_on_review`, fires once on the review-complete transition). Both opt-in, advisory, fire-and-forget. Possible follow-ups: a time-based debounce/coalesce if rapid review marks ever cause redundant pushes (the transition guard already prevents per-file spam), and surfacing an in-progress auto-sync indicator in the UI (today it's silent + logged).
